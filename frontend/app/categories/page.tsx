...
export default function CategoriesPage() {
  ...
  const createMut = useMutation({
    mutationFn: (name: string) => api.post("/complaint-categories", { name: name.trim(), isActive: isActiveDraft }),
    onSuccess: () => {
  ...
  const updateMut = useMutation({
    mutationFn: ({ id, name, isActive }: { id: number; name: string; isActive: boolean }) =>
      api.patch(`/complaint-categories/${id}`, { name: name.trim(), isActive }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["complaint-categories"] });
        ...
        >
          ...
          <div className="flex items-center justify-between rounded-xl border border-secondary-200 bg-secondary-50/50 p-4 transition-colors hover:bg-secondary-50 dark:border-white/10 dark:bg-white/5">
            <div className="space-y-0.5">
              ...
              <p className="text-xs text-muted-foreground">Inactive categories are hidden from new tickets.</p>
            </div>
