...
import { FacilityDepartment } from "@/types";
...
export default function DepartmentsPage() {
  ...
  const [inactiveTarget, setInactiveTarget] = useState<FacilityDepartment | null>(null);
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["facility-departments", "paged", search, activeFilter, page, pageSize],
    queryFn: async () => {
      ...
      const res = await api.get("/facility-departments", { params });
      return {
        list: (res.data?.data ?? []) as FacilityDepartment[],
  ...
  const toggleMut = useMutation({
    mutationFn: (id: number) => api.patch(`/facility-departments/${id}/toggle`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["facility-departments"] });
  ...
  const toggleRow = (d: FacilityDepartment) => {
    if (d.isActive) setInactiveTarget(d);
    else toggleMut.mutate(d.id);
  };
    ...
    <div className="p-6 space-y-6">
      ...
      <Card className="overflow-hidden border-secondary-200 bg-white shadow-sm">
        <div className="border-b border-secondary-100 px-6 py-4">
          <h3 className="text-lg font-bold text-secondary-900">All departments ({totalCount})</h3>
        </div>
          ...
          <table className="w-full whitespace-nowrap text-left text-sm">
            ...
            <tbody>
              {isLoading ? (
                ...
                rows.map((d, idx) => (
                  <tr key={d.id} className="border-b border-secondary-100 transition-colors hover:bg-primary-50/30">
                    <td className="px-4 py-3 text-center text-secondary-500">{totalCount - (page - 1) * pageSize - idx}</td>
                    <td className="px-4 py-3 font-semibold text-secondary-900">{d.name}</td>
                    <td className="px-4 py-3 text-secondary-700">{d.locationName ?? "—"}</td>
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="flex justify-end gap-1">
                        {canManage && (
                          <Button variant="ghost" size="sm" className="h-8 w-8 p-0" onClick={() => openEdit(d)} disabled={!canEdit}>
                        ...
                        {canManage && (
                          ...
                          >
                            {d.isActive ? <Ban className="h-4 w-4 text-amber-500" /> : <CheckCircle className="h-4 w-4 text-green-500" />}
                          </Button>
                        )}
                      </div>
                    </td>
        ...
        <div className="space-y-4 p-1">
          <div>
            ...
            <Input
              ...
              placeholder="e.g. Housekeeping, Engineering"
            />
          </div>
      ...
      <Dialog isOpen={!!inactiveTarget} onClose={() => setInactiveTarget(null)} title="Deactivate department" size="sm">
        <div className="space-y-4">
          <p className="text-sm text-secondary-600">
            Deactivate <span className="font-semibold text-secondary-900">{inactiveTarget?.name}</span>?
          </p>
